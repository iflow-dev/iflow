...
def i_check_database_state(step):
    # This step is mainly for documentation - the actual verification
    # happens in the "Then" steps
    pass


# Use existing step: "I see {count:d} search results" from
...
def the_artifact_should_have_property(step, property, value):
    ...
    # Look for the artifact with the specified property value in the
    # text content
    found = False
    ...
    assert found, (
        f"Artifact with {property} '{value}' not found in the database"
    )


# Use the generic property verification step: "the artifact should have
# {property} {value:QuotedString}"


# Use existing step: "the artifact {should|should not} be flagged" from
...
def the_repository_should_have_tag(step, tag):
    ...
    try:
        ...
        assert tag in tags, (
            f"Tag '{tag}' not found in repository. Available tags: {tags}"
        )
...
def the_repository_should_have_exactly_count_commits(step, count):
    ...
    try:
        ...
        assert actual_count == count, (
            f"Expected {count} commit(s), but found {actual_count}"
        )
